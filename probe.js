...
function cleanTiles(arr, start) {
  if (!Array.isArray(arr)) return [];
  const out = [];
  let id = start;
  for (const t of arr) { const c = cleanTile(t, id); if (c) { out.push(c); id++; } }
  return out;
}
...
function probeScore(rawRuleset, rawCtx) {
  ...
  const hand = cleanTiles(rawCtx.hand, 0);
  let nextId = hand.length;
  const melds = (Array.isArray(rawCtx.melds) ? rawCtx.melds : []).map(m => {
    const type = ['pong', 'kong', 'chow', 'concealed-kong'].includes(m && m.type) ? m.type : 'pong';
    const tiles = cleanTiles(m && m.tiles, nextId);
    nextId += tiles.length;
    return { type, tiles };
  }).filter(m => m.tiles.length >= 3);
  const flowers = cleanTiles(rawCtx.flowers, 1000);
