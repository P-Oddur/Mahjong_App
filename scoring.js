...
function scoreWinMcr(ctx, ruleset, handTiles, melds) {
  ...
  if (hasWinningShape && value === 0) {
    value = 8;
    breakdown = [{ id: 'chicken-hand', name: 'Chicken Hand', cn: '無番和', points: 8 }];
  }
...
function scoreWin(ctx, ruleset = DEFAULT_SCORING) {
  ...
  if (melds.length === 0) {
    if (isEnabledId('thirteen-orphans', ruleset) && isThirteenOrphans(handTiles)) candidates.push(scoreThirteenOrphans(cfg));
