...
// 17. 七對 (Seven Pairs): seven distinct pairs, mixed suits → 4 faan + 門前清 (concealed) = 5.
check('七對 (seven pairs) concealed = 5 faan', () => {
  const hand = [...pair('man', 2), ...pair('man', 5), ...pair('pin', 3), ...pair('pin', 7),
                ...pair('bam', 1), ...pair('bam', 9), ...pair('dragon', 'red')];
  const s = scoreWin({ hand, melds: [], seatWind: 'east', roundWind: 'east', selfDraw: false }, cfg);
  assert.strictEqual(s.faan, 5, `got ${s.faan}`); // 七對 4 + 門前清 1 (a special hand is fully concealed)
  assert.ok(ids(s).includes('seven-pairs') && ids(s).includes('concealed'));
});

// 18. 七對 stacks with 混一色 + 自摸 + 門前清.
check('七對 + half flush + self-draw + concealed = 9', () => {
  ...
  // seven-pairs(4) + half-flush(3) + self-draw(1) + 門前清(1) = 9 (HK 不求人 is default-off)
  assert.strictEqual(s.faan, 9, `got ${s.faan}`);
