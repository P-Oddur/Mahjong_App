...
check('MCR 七對 self-draw adds 自摸 (+1 pt)', () => {
  ...
  assert.ok(!ids(base).includes('self-draw'), 'no self-draw on a discard win');
  assert.ok(ids(sd).includes('self-draw'), 'self-draw fan present');
  assert.strictEqual(sd.value, base.value + 1, `self-draw adds 1 (base ${base.value}, sd ${sd.value})`);
});
...
check('MCR 組合龍 self-draw adds 自摸', () => {
