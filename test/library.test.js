...
check('seven-pairs base faan is overridable', () => {
  ...
  assert.strictEqual(s.faan, 7, `got ${s.faan}`); // 6 (override) + 門前清 1 (concealed)
  assert.strictEqual(s.breakdown.find(b => b.id === 'seven-pairs').faan, 6, 'override applied');
});

// 8. Disabling seven-pairs (via its allow flag — the authoritative knob since the
// sanitizer keeps patterns.enabled in lockstep with allow) makes the hand score nothing.
check('disabling seven-pairs drops the special hand', () => {
  const hand = [...pair('man', 2), ...pair('man', 5), ...pair('pin', 3), ...pair('pin', 7),
    ...pair('bam', 1), ...pair('bam', 9), ...pair('dragon', 'red')];
  const s = scoreWin(ctxOf(hand), sanitizeRuleset({ allow: { sevenPairs: false } }));
...
check('disabling all-honors un-limits seven honour pairs', () => {
  ...
  assert.strictEqual(s.faan, 5, `got ${s.faan}`); // plain seven pairs 4 + 門前清 1
});
