...
//    gesture. main.js calls initAudio() during boot, which creates the (often
//    'suspended') context; one-shot gesture listeners then UNLOCK on the first
//    real interaction — resuming the context, fading the master bus in, and
//    starting the ambient bed (INTEGRATION §14) — so SFX/ambience work without
...
let ambientStopTimer = null; // pending sub-bus disconnect timer (cleared on re-stop)
...
function bindGestureUnlock() {
  ...
  const unlock = () => {
    tryResume();
    fadeMasterIn();   // bring the master bus up smoothly on the first interaction
    startAmbient();   // §14: the ambient parlor bed starts on first gesture
...
// Smoothly raise the master bus from silence to its target level (or to 0 if
// muted). Used by the gesture unlock so sound fades in instead of popping.
function fadeMasterIn() {
  ...
  try {
    ...
    if (target <= 0) { masterGain.gain.setValueAtTime(0, now); return; }
    masterGain.gain.setValueAtTime(Math.max(0.0001, masterGain.gain.value), now);
    masterGain.gain.exponentialRampToValueAtTime(target, now + 0.6);
  } catch (e) {
...
export function initAudio() {
  ...
  masterGain.gain.value = 0; // silent until the first gesture fades it in (§14)
  masterGain.connect(ctx.destination);

  // The context often starts 'suspended' under autoplay policy. The first real
  // gesture resumes it, fades the master in, and starts the ambient bed.
  tryResume();
  bindGestureUnlock();
}
...
export function playTileClick() {
  ...
  // Disconnect the local bus once the LONGEST-lived node (the pitched tick, which
  // stops after the noise burst) ends, so the click's tail isn't cut early and the
  // bus is reliably freed — an oscillator's onended fires reliably, unlike a tiny
  // noise buffer's.
  const cleanup = () => { try { clickGain.disconnect(); } catch (e) {} };
  tick.onended = cleanup;
}
...
export function stopAmbient() {
  ...
  // Disconnect the sub-bus after the fade completes. The timer is tracked so a
  // rapid stop→start→stop can't leave an orphaned disconnect pending.
  if (ambientStopTimer) clearTimeout(ambientStopTimer);
  ambientStopTimer = setTimeout(() => {
    ambientStopTimer = null;
    try { busToKill.disconnect(); } catch (e) {}
  }, (FADE + 0.1) * 1000);
