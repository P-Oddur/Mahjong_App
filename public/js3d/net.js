...
export function doRejoin() {
  let s = {};
  // Guarded: this runs on every 'connect', so corrupt sessionStorage must not throw
  // inside the connect handler and silently kill rejoin for the whole session.
  try { s = JSON.parse(sessionStorage.getItem('mahjong') || '{}') || {}; } catch (_) {}
...
export function emitNextHandVote(value)       { socket && socket.emit('nextHandVote', { value }); }
...
export function emitOrganizeHand()            { socket && socket.emit('organizeHand'); }
export function emitShuffleHand()             { socket && socket.emit('shuffleHand'); }
